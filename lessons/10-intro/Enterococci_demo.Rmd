...
Data from OEH Beach water quality data, available [here](https://www.environment.nsw.gov.au/topics/water/beaches/beach-water-quality-data). Data for all of NSW can be downloaded at [the download tab](https://www.environment.nsw.gov.au/beachapp/report_enterococci.aspx). 
...
filter(x, Region ==  "Sydney City Ocean Beaches", !is.na(`Enterococci (cfu/100ml)`)) %>% write_csv("data/sydneybeaches.csv")
-->
...
data <- read_csv("data/sydneybeaches.csv") %>% 
  ...
  filter(!is.na(`Enterococci (cfu/100ml)`)) 
```
...
```{r, warning=FALSE}
data %>% 
  ggplot(aes(y = `Enterococci (cfu/100ml)`, x = year, colour = log(`Enterococci (cfu/100ml)`))) +
...
```{r}
data_randwick %>% 
  ggplot(aes( x = rain, y = `Enterococci (cfu/100ml)`, colour = log(`Enterococci (cfu/100ml)` ))) +
    geom_point() +
    scale_y_log10( name="Enterococci (cfu/100ml)") + 
...
data_randwick %>% 
  ...
  ggplot(aes( x = rain, y = `Enterococci (cfu/100ml)`, col = Site)) +
    geom_point() +
    scale_y_log10( name="Enterococci (cfu/100ml)")+ 
    scale_x_log10( name="Rainfall (mm)") +
