...
data %>% 
  filter(year == 1950) %>%
  ggplot(aes(gdp_per_capita, fertility, col = continent)) + 
  geom_point() +
  scale_x_log10()
```
...
```{r}
data %>% 
  filter(country %in%  c("Australia", "Angola")) %>%
  ggplot(aes(gdp_per_capita, fertility, col = country)) + 
  geom_line(col = "blue") + geom_point(size = .5, col = "darkblue") + 
  theme_minimal()
```
```
...
data %>% 
  filter(year==1990) %>%
  ggplot(aes(gdp_per_capita, fertility, colour = continent, size=population)) + 
  geom_point() + 
...
data %>% 
  filter(
    year %in% c(1800,1850,1900,1950,2000),
    !is.na(continent)) %>%
  ggplot(aes(gdp_per_capita, fertility, colour = continent)) + 
  theme_classic()+
  geom_point(alpha = 0.7) + 
  scale_color_viridis(discrete = TRUE) + 
  geom_smooth(aes(col="mean"), se =FALSE, alpha = 0.5, method = "lm", size = 0.5) +
  scale_x_log10() +
  xlab("GDP percapita (USD)") + 
