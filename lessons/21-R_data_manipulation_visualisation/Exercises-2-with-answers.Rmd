...
This dataset tells a grand story about human development across countries, and was made famous in TED talks by Hans Rolsing -- eg. see https://www.ted.com/talks/hans_rosling_the_best_stats_you_ve_ever_seen#t-126589 (start 2:10)
...
I'd porbably read in the csv tables and join them straight in, without saving as intermiedate objects
...
2. The journal where you are submitting your paper wants a table summarising the dataset, giving the 
  ...
  - time frame
  - mean of vairables fertility and life_expectancy
...
When you want to run an opertion on subgroups in the data, we turn to `dplyr's` `group_by` command. This turbocharges other commands like `summarise` & `mutate`. 
...
Group_by can also be used to calculate mutations at the group level. Let's image you wanted to caluclate each countries population as a percentage of its current population. 
...
**Exercise**: Use the comands `group_by`  command to
...
Instead of an ifelse, we could also use the na_replace function
