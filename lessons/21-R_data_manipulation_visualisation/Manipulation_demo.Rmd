...
We have extracted a dataset for Sydney city, stored in `data/sydney_beaches/sydneybeaches.csv`.

<!-- 
x <- read_csv( "EnterococciDataDownload-20181218-090215.csv") %>% 
filter(x, Region ==  "Sydney City Ocean Beaches", !is.na(Enterococci_cfu_per_100ml)) %>% write_csv("data/sydney_beaches/sydneybeaches.csv")
-->

Load data, extract year using the package lubridate

```{r,warning=FALSE, results='hide', message=FALSE}
data <- read_csv("data/sydney_beaches/sydneybeaches.csv") %>%
  mutate(
...
leaflet() %>%
  addTiles() %>%
  addMarkers(
    lng = sites$Longitude, lat = sites$Latitude,
...
```{r}
rain <- read_csv("data/sydney_beaches/randwickrain.csv") %>%
  mutate(
