...
Load some super useful packages
...
Load data, extract Date and Year using the package lubridate

```{r,warning=FALSE, results='hide', message=FALSE}
data <- read_csv("data/sydneybeaches.csv")
```

Reload data, only keep complete reocrds, extract Date and Year using the package lubridate:
...
Do we have all dates for all beaches?

```{r}
data %>% 
  group_by(Site) %>%
  summarise(start = min(Date), end = max(Date), count = n()) %>% 
  arrange(start)
```

Let's plot locations:
