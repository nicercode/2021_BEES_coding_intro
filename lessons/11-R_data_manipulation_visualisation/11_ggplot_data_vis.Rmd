...
```{r}
#Density/Histograms
ggplot(data = movies, aes(x = domestic_gross)) + 
  geom_histogram(aes(y = ..density..), fill = "light blue") +
  geom_density() 

#Scatter plots for exploring relationships
ggplot(data = movies, aes(x = production_budget, y = domestic_gross)) +   geom_point(alpha = 0.2) 
...
ggplot(data = movies, aes(x = genre, y = production_budget)) + 
  geom_violin(trim = F) + 
  geom_point(colour = "light blue", size = 1, alpha = 0.1) + 
  stat_summary(fun.data="data_summary", col = "black") 

#Quick y axis transform
ggplot(data = movies, aes(x = genre, y = production_budget)) + 
  geom_boxplot() + 
  scale_y_log10()
```
...
#Axis label size/type face/font
ggplot(data = movies, aes(x = genre, y = production_budget)) + 
  geom_boxplot() +
  ylab("Production budget (USD)") + 
  xlab("Genre") + 
